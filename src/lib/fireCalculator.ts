...
// ─────────────────────────────────────────────────────────────────────────────
// ЕДИНИЦЫ ДЕПРЕССИИ. Сопротивление ветви (b.resistance) хранится в СИ —
// Н·с²/м⁸: перевод из рудничных кМюрг (×9,81) выполнен ОДИН РАЗ в
// calcResistance (см. aerodynamics.ts, KMURG_TO_SI). Поэтому депрессия
// b.dP = R·Q² сразу в ПАСКАЛЯХ и напрямую сопоставима с тепловой депрессией
// пожара, естественной тягой и напором вентилятора из каталога.
// Здесь никаких дополнительных множителей быть НЕ должно — иначе перевод
// применится дважды и критическая депрессия завысится ~в 10 раз.
//
// Формулы (5.3)–(5.5) и Прил. 7 берут R из тех же ветвей, поэтому дают h_кр
// сразу в паскалях — сравнение с тепловой депрессией корректно.
