...
// ─────────────────────────────────────────────────────────────────────────────
// ЕДИНИЦЫ ДЕПРЕССИИ. Депрессия ветви (b.dP = R·Q² из расчёта сети) уже выражена
// в ПАСКАЛЯХ и напрямую сопоставима с тепловой депрессией пожара, естественной
// тягой и напором вентилятора — дополнительный перевод НЕ нужен.
// Проверено по эталону АэроСети: коэффициент сопротивления крепи задаётся с
// множителем 10⁻⁴, и для типовой сети (ствол α=5, уклон α=30, штреки α=80…100)
// суммарное R ≈ 0,24 даёт рабочую точку ВО-18/12АВР 60 м³/с при 877 Па — точно
// в паспортной зоне (20…82, номинал 63). Если трактовать R как кгс·с²/м⁸ и
// умножать депрессию на 9,81, рабочая точка уходит на 27,5 м³/с — нефизично
// для ГВУ. Множитель 9,81 встречается только в пересчёте ЗАМЕРОВ перемычки
// (bulkheadSurveyDP) — это частный случай, а не общее правило.
// НЕ добавлять сюда перевод в Па: это завышает критическую депрессию ~в 10 раз
// и делает заведомо неустойчивые выработки «устойчивыми».
