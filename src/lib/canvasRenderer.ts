  ...
  for (const { b } of group2) {
    ...
    if (showDashes && segLen > 24) {
      ...
      if (segLen > arrowLen) {
        ...
        // воздуха V. Раньше задавалось время цикла, а расстояние за цикл
        // зависело от толщины линии и длины выработки: толстая выработка с
        // медленным воздухом обгоняла тонкую с быстрым. Теперь то, что глаз
        // читает как скорость (пиксели в секунду), честно отражает воздух.
        // Формула и пределы — те же, что в SVG-режиме, чтобы вид совпадал.
        const pxPerSec = Math.max(12, Math.min(400, V * 22)) * Math.max(0.1, animSpeed);
        const dur = Math.max(0.15, runLen / pxPerSec);
