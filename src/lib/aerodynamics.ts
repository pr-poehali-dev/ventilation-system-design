...
// Рудничные формулы дают R в кМюрг (кгс·с²/м⁸) — так же, как АэроСеть.
// Решатель считает депрессию ΔP = R·Q² в паскалях, поэтому R переводится в
// СИ (Н·с²/м⁸) умножением на 9,81 (1 кгс = 9,81 Н).
// ─────────────────────────────────────────────────────────────────────────────
export const KMURG_TO_SI = 9.81;
...
export function calcResistance(i: ResistanceInput): {
  ...
  // alpha уже содержит ρ (единицы α: кгс·с²/м⁴) — масштабируем
  if (i.mode === "alpha" || i.mode === "surface") {
    Rfriction = Rfriction * rhoFactor;
  }

  // ── ЕДИНИЦЫ: кМюрг → Н·с²/м⁸ ──────────────────────────────────────────────
  // Все формулы выше (α·P·L/S³, λ·L·P/8S³, 6,48·α·L/D⁵) — рудничные и дают R
  // в кМюрг (кгс·с²/м⁸), как в АэроСети. Ручной ввод R пользователь тоже задаёт
  // в кМюрг. Решатель же считает депрессию как ΔP = R·Q² в ПАСКАЛЯХ, поэтому
  // сопротивление надо перевести в СИ: 1 кгс = 9,81 Н → ×9,81.
  //
  // Раньше перевода не было: депрессии всей сети занижались в 9,81 раза
  // (ствол 0,00247 кМюрг при Q=85 давал 18 Па вместо 177 Па, как в АэроСети),
  // а чтобы расход не «раздувался» при таком слабом сопротивлении, были
  // искусственно занижены и характеристики вентиляторов. Обе ошибки гасили
  // друг друга лишь в одной точке — на других схемах сеть выходила «зажатой».
  Rfriction = Rfriction * KMURG_TO_SI;

  // Местные ξ → R_local = ξ·ρ/(2·S²) — эта формула СРАЗУ в СИ (Н·с²/м⁸),
  // т.к. ΔP_loc = ξ·ρ·V²/2 [Па]. Дополнительный перевод ей НЕ нужен.
