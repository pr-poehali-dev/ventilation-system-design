...
export const VENT_DUCT_BRANDS: VentDuctBrand[] = [
  {
    ...
    // Проверка по таблице 2 изготовителя (R = 6,48·α·L/D⁵) показывает, что
    // сама таблица посчитана по α ≈ 2,55, а не 2,4: обратный пересчёт всех
    // 17 строк даёт разброс 2,46…3,15 со средним около 2,55. Берём 2,55 —
    // тогда расчёт совпадает с таблицей изготовителя, а не занижает её на
    // 4–24 %, как было бы при 2,4. Расхождение с заявленным 0,00024 —
    // округление в исходном документе.
    alpha: 2.55,
    antistatic: true,
  },
];
