...
// Сопротивление перемычки с РЕГУЛИРУЕМЫМ ОКНОМ (кМюрг). Формула истечения через
// отверстие (диафрагму): перепад определяется СКОРОСТЬЮ ВОЗДУХА В ОКНЕ (Q/Sок),
// а сопротивление приводится к кМюрг (кгс·с²/м⁸ — как остальные формулы и как
// АэроСеть) делением на g:
//   R = ρ / (2·μ²·Sок²·g)
// где ρ=1,2 кг/м³, μ=0,75, g=9,80665. Проверка (эталон АэроСети):
//   Sок=5,5 → R≈0,0036 кМюрг (точно совпадает с АэроСети).
...
export function windowBulkheadRkMurg(windowArea: number, _sectionArea?: number): number {
  ...
  return 1.2 / (2 * WINDOW_MU * WINDOW_MU * Sok * Sok * 9.80665);
}
