...
// перемычки». Откалиброван по ДВУМ эталонам «Аэросеть» (формула — диафрагма с
// учётом скорости подхода, результат делится на g → кМюрг):
//   ВЦД-47:  ΔS=17,35, S=38,5 → R=0,00464 кМюрг  ⇒ μ=0,187
//   AL25-4800: ΔS=4,91, S=38,5 → R=0,0622 кМюрг  ⇒ μ=0,200
// Принято среднее μ=0,193: оба эталона воспроизводятся с погрешностью <8 %.
export const FAN_WINDOW_MU = 0.193;
...
export function fanWindowRkMurg(windowArea: number, sectionArea?: number): number {
  ...
  const S = sectionArea && sectionArea > 0 ? sectionArea : 0;
  const approach = S > 0 ? 1 / (S * S) : 0;
  // ρ/(2μ²)·(1/ΔS² − 1/S²) — формула диафрагмы, ρ в кг/м³ ⇒ результат в СИ
  // (Н·с²/м⁸). Делим на g, чтобы вернуть кМюрг — как windowBulkheadRkMurg и
  // solidBulkheadRkMurg. Решатель домножает сумму перемычек обратно на 9,81.
  // БЕЗ этого деления окно ГВУ получало сопротивление в 9,81 раза больше и
  // «душило» установку: расход ствола падал с 85 до 48 м³/с.
  const r = 1.2 / (2 * G_ACCEL * FAN_WINDOW_MU * FAN_WINDOW_MU) * (1 / (dS * dS) - approach);
  return r > 0 ? r : 0;
}
