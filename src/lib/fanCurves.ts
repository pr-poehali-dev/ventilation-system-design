...
export const FAN_CATALOG: FanCurve[] = [
  ...
  },
  {
    ...
    // Паспортная характеристика при 490 об/мин (эталон АэроСеть):
    //   H(160)≈4560, H(480)≈3900, рабочая точка ГВУ Q=724.2 → H=2731 Па.
    //   H = 4518 + 1.034·Q − 0.004838·Q²
    // Мощность на графике 1200..1840 кВт; КПД пик ~74% (Q≈360),
    //   в рабочей точке Q=724, H=2731, N≈5028 кВт → η=0.39 (АэроСеть ✓).
    // η(Q) = 0.3218 + 0.002204·Q − 2.92e-6·Q²
    id: "VC-47",
    name: "ВЦД-47У", type: "centrifugal", diameter: 4.7,
    h0: 4518.46, h1: 1.03365, h2: -0.0048377,
    e0: 0.32178, e1: 0.0022040, e2: -0.0000029204,
    qMin: 150, qMax: 760, qNominal: 724, hNominal: 2731,
  ...
  },
  {
    ...
    // Коэффициенты подобраны под ПАСПОРТНЫЕ характеристики ВО-18/12АВР (1500 об/мин)
    // по 5 углам лопаток 30/35/40/45/50°. Модель — единая кривая H=h0·af+h1·Q+h2·Q²
    // с множителем угла af=0.65+((angle−20)/30)·0.70 (не изменялась). Так как одна
    // кривая физически не описывает точно все 5 углов (диапазон расхода 39→120 м³/с),
    // коэффициенты подобраны методом наименьших квадратов с приоритетом РАБОЧЕЙ зоны
    // (левая + номинальная точки). rpmNominal=1500 → k=1 на паспортных оборотах.
    // Рабочая зона: 30°≈23–39, 40°≈49–81, 50°≈72–120 м³/с. Напор в номинале
    // совпадает с паспортом с точностью ~±300 Па в середине диапазона.
    h0: 4150, h1: -14.26, h2: -0.03,
    e0: 0.3669, e1: 0.00985, e2: -0.0000728,
    qMin: 20, qMax: 90, qNominal: 65, hNominal: 3400,
    rpmMin: 600, rpmMax: 1500, rpmNominal: 1500,
    bladeAngles: [20, 25, 30, 35, 40, 45, 50],
    // Реверс ВО-18/12АВР: единая падающая кривая, подобрана по паспортным
    // реверс-точкам углов 35–50° (МНК, RMSE≈580 Па). Диапазон Q≈28–110 м³/с.
    reverseH0: 3035.7, reverseH1: -25.04, reverseH2: -0.01,
    reverseQMin: 28, reverseQMax: 110,
    reverseEfficiencyFactor: 0.82,
  },
  {
    id: "VOD-21",
