  ...
  for (; iter < maxIter; iter++) {
    ...
    for (const contour of contours) {
      ...
      for (const { edgeIdx, dir } of contour) {
        ...
        // Вклад вентилятора по методу Кросса:
        // ΔH_k = Σ R·Qd·|Qd| − Σ H_eff·sign(Qd)
        // H_eff = +H прямой режим, H_eff = -H реверс (нагнетание b→a)
        if (e.hasFan) {
          // Метод Кросса: вентилятор вычитается из невязки контура со знаком
          // знака фактического расхода в направлении нагнетания вентилятора.
          //   H_eff = H при прямом режиме (нагнетание a→b)
          //   H_eff = -H при реверсе (нагнетание b→a)
          // Вклад: num -= H_eff * sign(Qd)
          // sign(Qd) = знак расхода в направлении обхода контура
          const H       = fanH(e, e.Q);                    // |H| >= 0
          const H_eff   = e.fanReverse ? -H : H;           // эффективный напор с учётом направления нагнетания
          const signQd  = Qd > 0 ? 1 : Qd < 0 ? -1 : 0;
          num -= H_eff * signQd;
          den += fanDH(e, e.Q);
        }
