 ...
 */
/**
 * Оценка начального расхода Q₀ по рабочей точке главного вентилятора.
 *
 * ЕДИНИЦЫ: Rtotal приходит в кМюрг, напор H — в Па. Поэтому сопротивление
 * сети сразу переводим в Па (×9,81), иначе Q₀ завышается примерно втрое и
 * итерации стартуют далеко от решения.
 */
function estimateQ0(edges: Edge[], RtotalKmurg: number): number {
  const Rtotal = RtotalKmurg * PA_PER_MM_H2O;
...
export function solveNetwork(
    ...
    return {
      ...
      R:             Math.max(MIN_R, b.resistance + (b.hasBulkhead ? (() => {
        ...
        if (mode === "survey") {
          ...
          // R = ΔP/(Q²·9.81) в кМюрг. ΔP вводится в Па, а кМюрг определён через
          // кгс/м² (мм вод.ст.), 1 кгс/м² = 9.81 Па. Делим на 9.81 — как остальные
  ...
  for (; iter < maxIter; iter++) {
    ...
    for (const contour of contours) {
      ...
      for (const { edgeIdx, dir } of contour) {
        ...
        // Напор вентилятора H — в Па. Без множителя 9,81 невязка складывала
        // мм вод. ст. с Па: сопротивление сети занижалось в 9,81 раза, расход
        // получался завышенным, а сумма депрессий по контуру не сходилась в ноль.
        num += e.R * Qd * Math.abs(Qd) * PA_PER_MM_H2O;
        ...
        // Производная невязки по расходу — в тех же единицах (Па), что и num.
        den += 2 * e.R * Math.abs(Qd) * PA_PER_MM_H2O;
      }
  ...
  // Узлы, недостижимые по потоку (закрытые ляды, перемычки, тупики).
  // При Q=0 депрессия ветви НЕ нулевая: закрытый ствол ЮВС имеет Q=0, но
  // депрессию 694 Па (аэростатический столб на 222 м). Поэтому давление соседа
  // не копируется, а пересчитывается через реальную депрессию ветви.
  const pQueue2 = [...pVis];
  ...
  // ДЕПРЕССИЯ УЗЛА (как в АэроСети).
  // Ноль на устье свежей струи, по ходу воздуха растёт по модулю, у ГВУ равна
  // полной депрессии шахты. Разность соседних узлов = депрессия их ветви.
