            ...
            return (
              <g key={sym.id} data-sym={sym.id}
                ...
                {sym.branchId && hasBranchPts && sym.typeId !== "valve_reduce" && (() => {
                  ...
                  // (габарит ≈ pw = ph·0.38, у двери +gap), поэтому подложка
                  // размером SZ+uW «вылезала» на соседние перемычки и перекрывала
                  // их. Берём реальный габарит символа вдоль ветви + запас на обводку,
                  // чтобы подложка не залезала на соседей. Для остальных символов
                  // (иконки, вентиляторы) оставляем прежний размер SZ+uW.
                  const uLen = isBulkheadOv
                    ? Math.max(uW, SZ * 0.85 * 0.38 * 2.6 + uW)
                    : Math.max(uW, SZ + uW);
