...
def compute_node_pressures(edges, Q, nodes_in):
    ...
    #
    # У самого вентилятора — полная депрессия шахты, по ходу движения воздуха
    # к устью свежей струи она убывает по модулю на потери каждой ветви.
    # При этом сохраняется главное свойство: разность депрессий соседних узлов
    # равна депрессии связывающей их ветви (знак «минус» сокращается).
    #
    # Проверка на схеме пользователя (напор ГВУ 1727 Па):
    #   узел Z=599: наши потери 167 Па → −(1727 − 167) = −1560 (АэроСеть −1575);
    #   узел Z=377 ниже по стволу: разница ≈ 694 Па — депрессия ствола ЮВС.
    main_fan_H = 0.0
    ...
    if _main:
        # Если ГВУ несколько (параллельная работа) — берём максимальный напор,
        # это и есть полная депрессия шахты.
        main_fan_H = max(fan_H(e, abs(Q.get(e["id"], 0.0))) for e in _main)
