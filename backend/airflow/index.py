...
def compute_node_pressures(edges, Q, nodes_in):
    ...
    #
    # Ноль на устье воздухоподающего ствола, по ходу воздуха значение растёт по
    # модулю и достигает максимума у ГВУ (равно напору вентилятора).
    # Главное свойство: разность депрессий соседних узлов равна депрессии
    # связывающей их ветви.
    #
    # Проверка на схеме пользователя (ЮВС):
    #   верх ствола −1570 Па + депрессия ствола 690 Па = −880 → в узле −879 Па.
    #   на устье −1571 Па, напор ГВУ 1727 Па.
    ...
    for n in nodes_in:
        ...
        # Депрессия = накопленные потери от устья (отрицательная на всасывании)
        p_fan = round(P - P_ATM)
