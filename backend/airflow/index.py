...
def natural_draft_h(from_z, to_z, from_temp, to_temp):
    """
    Естественная тяга ветви (Па) — вклад веса столба воздуха ветви.
    H_nat = g * (z_from - z_to) * ρ_ветви
    где ρ_ветви = 353 / (273 + T_ср) — средняя плотность воздуха в ветви (кг/м³).

    ВАЖНО: это КОНСЕРВАТИВНАЯ (потенциальная) величина — при обходе замкнутого
    контура вклады телескопируются. Прежняя формула (ρ_to − ρ_from) НЕ была
    потенциальной: для двух стволов с ОДИНАКОВЫМ профилем температур вклады не
    сокращались, а СКЛАДЫВАЛись, создавая ложную тягу ~−575 Па (баг: при реверсе
    расход подскакивал, хотя обе устья на одной отметке → тяга должна быть 0).

    Для контура из двух стволов A(вниз) и B(вверх):
      Σ = g·(z_в−z_н)·ρ_A + g·(z_н−z_в)·ρ_B = g·(z_в−z_н)·(ρ_A − ρ_B)
    → если температуры (плотности) стволов равны, тяга контура = 0.
    """
    g = 9.81
    t_mean   = (from_temp + to_temp) / 2.0
    rho_mean = 353.0 / (273.0 + max(-60.0, min(100.0, t_mean)))
    return g * (from_z - to_z) * rho_mean
