...
def natural_draft_h(from_z, to_z, from_temp, to_temp):
    ...
    ВАЖНО: используем именно разность плотностей (ρ_to − ρ_from), а не вес
    столба ρ_ср. Вес столба даёт огромные величины (~3500 Па на ствол),
    которые при коллапсе устьев в единый узел GND перестают телескопироваться
    и опрокидывают баланс вентилятора (Q ГВУ шёл в обратную сторону, соседний
    ствол принимал воздух вместо выдачи). Разность плотностей сразу даёт малую
    физическую тягу (~100–200 Па на ствол), согласованную с напором ГВУ.
    """
    g = 9.81
