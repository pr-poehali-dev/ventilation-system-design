...
def natural_draft_h(from_z, to_z, from_temp, to_temp, atm_temp=None):
    """
    Естественная тяга ветви (Па) — вес столба воздуха ветви ОТНОСИТЕЛЬНО
    атмосферного столба той же высоты:
        H_nat = g * (z_from − z_to) * (ρ_ветви − ρ_атм)
    где ρ_ветви = 353/(273 + T_ср) — средняя плотность воздуха в ветви,
        ρ_атм    = 353/(273 + T_поверхности) — плотность наружного воздуха.

    ПОЧЕМУ ОТНОСИТЕЛЬНО АТМОСФЕРЫ (а не (ρ_to − ρ_from) прежней версии):
    два ствола с разной высотой устьев (500 м и 200 м) соединены общей
    атмосферой. Замыкание контура идёт через узел GND, и атмосферный столб
    между устьями (z=500…200) даёт компенсирующий вклад. Прежняя формула
    (ρ_to − ρ_from) его НЕ учитывала → тяга в контуре завышалась (~−270 Па,
    Q=36.8 вместо 7.1 у АэроСети). Отняв ρ_атм, мы учитываем этот столб:
    при обходе контура g·Σ(z_from−z_to)·(−ρ_атм) телескопирует атмосферный
