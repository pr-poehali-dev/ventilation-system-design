...
def get_R(b):
    """Сопротивление ветви. Единицы согласованы с напором вентилятора (Па):
    R·Q² сразу даёт депрессию в паскалях, как и на ветвях в интерфейсе.
    Дополнительных переводов делать НЕ нужно — попытка домножить на 9,81
    «зажимала» сеть и занижала расход у вентиляторов в 2–2,5 раза.
    """
...
def compute_node_pressures(edges, Q, nodes_in):
    ...
    # ══════════════════════════════════════════════════════════════════
    # ДЕПРЕССИЯ УЗЛА — ОТСЧЁТ ОТ ВЕНТИЛЯТОРА (как в АэроСети)
    # ══════════════════════════════════════════════════════════════════
    # У ГВУ депрессия равна полной депрессии шахты со знаком минус, и по мере
    # удаления от вентилятора ПРОТИВ хода воздуха она уменьшается по модулю на
    # депрессию пройденных участков, обращаясь в ноль на устье свежей струи.
    #
    #   depr(всас ГВУ)   = −H_гву
    #   depr(узел выше)  = depr(узел ниже) + депрессия_участка
    #
    # Сверка со схемой пользователя: H_гву=1726 Па, потери от ГВУ до сопряжения
    # 156 Па → на сопряжении −1726 + 156 = −1570 Па (в АэроСети −1570 Па).
    #
    # Почему прежний вариант (P − P_атм от устья вниз по потоку) давал −273 Па:
    # входной и выходной стволы объединены в один узел «атмосфера», поэтому на
    # разветвлённой сети обход мог прийти в сопряжение коротким путём со
    # стороны ИСХОДЯЩЕЙ струи и принести туда лишь малые потери приустьевого
    # участка вместо полной депрессии от ГВУ. Отсчёт от вентилятора этой
    # неоднозначности лишён: путь всегда идёт против потока от ГВУ.
    def _edge_loss(e, q):
        """Депрессия участка (только потери, без напора вентилятора), Па."""
        R = float(e.get("R", 0.0))
        h_nat = float(e.get("naturalDraft", 0.0) or 0.0)
        return R * q * abs(q) - h_nat * (1.0 if q >= 0 else -1.0)

    # Главные вентиляторы: ГВУ/ВВУ в работе. ВМП — местный, шкалу не задаёт.
    main_fans = [
        (i, e) for i, e in enumerate(edges)
        if e.get("hasFan") and not e.get("fanStopped")
        and e.get("fanType", "ГВУ") in ("ГВУ", "ВВУ")
    ]

    depr = {}
    if main_fans:
        dq = collections.deque()
        for ei, e in main_fans:
            q = Q.get(e["id"], 0.0)
            if abs(q) < 1e-9:
                continue
            H = fan_H(e, abs(q))
            # Всас — узел, из которого воздух ВХОДИТ в вентилятор,
            # нагнетание — узел, куда вентилятор воздух выбрасывает.
            suction, discharge = (e["a"], e["b"]) if q > 0 else (e["b"], e["a"])
            # На стороне нагнетания (устье исходящей струи) депрессия нулевая.
            if discharge not in depr:
                depr[discharge] = 0.0
                dq.append(discharge)
            # Перед вентилятором — полная депрессия шахты со знаком минус.
            # Из напора вычитаем потери самого участка с вентилятором (канал,
            # диффузор): иначе они учлись бы дважды и шкала не сходилась бы
            # в ноль на устье свежей струи.
            if suction not in depr:
                depr[suction] = -(H - abs(_edge_loss(e, q)))
                dq.append(suction)

        # Проход 1: строго ПРОТИВ хода воздуха (от вентилятора к устью).
        while dq:
            v = dq.popleft()
            for other, ei in adj[v]:
                if other in depr:
                    continue
                e = edges[ei]
                if e.get("hasFan"):
                    continue          # через вентилятор шкалу не продолжаем
                q = Q.get(e["id"], 0.0)
                if abs(q) < 1e-9:
                    continue
                # Идём вверх по потоку: воздух должен ВТЕКАТЬ в v из other.
                upstream = (e["b"] == v and q > 0) or (e["a"] == v and q < 0)
                if not upstream:
                    continue
                depr[other] = depr[v] + abs(_edge_loss(e, q))
                dq.append(other)

        # Проход 2: узлы вне основной струи (тупики, перемычки, Q=0) —
        # добираем по любой связи, вычитая депрессию участка.
        dq = collections.deque(depr.keys())
        while dq:
            v = dq.popleft()
            for other, ei in adj[v]:
                if other in depr:
                ...
                q = Q.get(e["id"], 0.0)
                dP = _edge_dP(e, q)
                depr[other] = depr[v] - dP if e["a"] == v else depr[v] + dP
                dq.append(other)
    ...
    for n in nodes_in:
        ...
        # ДЕПРЕССИЯ УЗЛА — отсчёт от вентилятора (см. блок выше): у ГВУ равна
        # полной депрессии шахты со знаком минус, к устью свежей струи убывает
        # по модулю на депрессию пройденных участков.
        #
        # Если главных вентиляторов нет (только естественная тяга или всё
        # остановлено) — шкала не определена, тогда прежний отсчёт P − P_атм.
        p_fan = round(depr[pid]) if pid in depr else round(P - P_ATM)
